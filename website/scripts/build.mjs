...
const assetRevision = '20260904-1';
...
const desktopRelease = {
  ...
  // Reserved for a publicly trusted stable Windows release, never a self-signed Preview.
  windowsInstaller: ''
};
const windowsPreview = {
  version: '0.6.1-preview.2',
  page: 'https://github.com/docflowlocal/docflow-desktop/releases/tag/v0.6.1-preview.2',
  installer: 'https://github.com/docflowlocal/docflow-desktop/releases/download/v0.6.1-preview.2/DocFlow-Local-Setup-0.6.1-preview.2-x64-Self-Signed-Preview.exe',
  installerSha256: 'e74c8d79e818338cdfb24ad7d227401e1932ae08ec2ae5c174e3349726bb23c8'
};
...
const pages = {
  en: {
    ...
    download: { path: '/download/', title: 'Download DocFlow Local | macOS & Windows Preview', description: 'Download DocFlow Local Community free: a signed, notarized macOS release or a separately labeled self-signed Windows x64 Preview. No registration required.' },
  ...
  zh: {
    ...
    download: { path: '/zh/download/', title: '下载 DocFlow Local｜macOS 正式版与 Windows 预览版', description: '免费获取 DocFlow Local Community：macOS 已签名、公证的正式版，或独立标注的 Windows x64 自签名预览版。无需注册，客户文件留在本机。' },
...
function downloadPage(locale) {
  ...
  const quickstart = zh ? [
    ['01','下载并安装','选择 macOS 正式版；Windows 测试用户请先阅读自签名预览版说明。无需创建账号。'],
    ['02','选择一个 Starter','从外贸报价、工程移交、HR 入职或合规材料开始。'],
    ['03','先生成，再换成我的文件','运行脱敏样例、修复预检问题，然后导入自己的表格和模板。']
  ] : [
    ['01','Download and install','Choose the macOS release, or read the self-signed Preview guidance before testing on Windows. No account is required.'],
  ...
  const windowsDownloadAction = desktopRelease.windowsInstaller
    ...
    : trackedButton({ href: windowsPreview.installer, label: zh ? '下载 Windows 自签名预览版' : 'Download Windows Self-Signed Preview', style: 'secondary', eventName: 'download_windows_preview', arrow: true, analytics: { ctaId: 'download_windows_self_signed_preview', ctaLocation: 'download_card', pageType: 'download', locale, destination: 'github_release_asset', platform: 'windows', assetType: 'exe', releaseVersion: windowsPreview.version } });
  const body = `${pageHero(locale,'download',zh?'macOS 正式版 · Windows 预览版':'macOS RELEASE · WINDOWS PREVIEW',zh?'免费下载 DocFlow Local Community':'Download DocFlow Local Community free',zh?`Apple Silicon Mac 可下载已签名、公证的 ${desktopRelease.version} 正式版；Windows x64 提供独立的自签名测试版。两者均支持中文与 English，无需注册。`:`Apple Silicon Macs can use the signed, notarized ${desktopRelease.version} release. Windows x64 has a separate self-signed test build. Both include English and Simplified Chinese, with no registration.`,false)}
  <section class="section"><div class="container"><div class="micro-trust download-promises">${promises.map(value=>`<span><i></i>${value}</span>`).join('')}</div><div class="download-grid"><article class="download-card"><span class="os-icon">⌘</span><h2>macOS</h2><p>${zh?'适用于 Apple Silicon（M 系列芯片），已使用 Apple Developer ID 签名并完成 Apple 公证。':'For Apple Silicon (M-series chips), signed with Apple Developer ID and notarized by Apple.'}</p>${macDownloadAction}</article><article class="download-card"><span class="os-icon">⊞</span><h2>Windows Preview</h2><p>${zh?'适用于 Windows 10/11 x64。当前为自签名测试版，不是公众可信签名的正式版。SmartScreen 或 Smart App Control 仍可能提示或阻止运行；请勿关闭系统安全保护。':'For Windows 10/11 x64. This is a self-signed test build, not a publicly trusted production release. SmartScreen or Smart App Control may warn or block it. Do not disable system security protections.'}</p>${windowsDownloadAction}<p><a href="${betaEmail[locale]}" ${analyticsAttributes('beta_request',{ ctaId: 'download_windows_beta', ctaLocation: 'download_card', pageType: 'download', locale, destination: 'windows_beta_email', platform: 'windows' })}>${zh?'获取可信签名正式版通知':'Get notified about the trusted stable release'}</a></p></article></div><div class="download-note"><strong>${zh?`macOS ${desktopRelease.version} 发布信息`:`macOS ${desktopRelease.version} release information`}</strong><br>${zh?'标准安装请下载 .pkg；如需便携归档可下载 .zip。安装包已完成 Apple 公证。PKG SHA-256：':'Download the .pkg for the standard installation flow or the .zip for a portable archive. The app is Apple notarized. PKG SHA-256: '}<code>${desktopRelease.macPkgSha256}</code><br><a href="${desktopRelease.page}" ${analyticsAttributes('view_release_details',{ ctaId: 'download_release_details', ctaLocation: 'release_note', pageType: 'download', locale, destination: 'github_release', platform: 'macos', releaseVersion: desktopRelease.version })}>${zh?'查看发布说明、ZIP、SBOM 与完整校验信息':'View release notes, ZIP, SBOM, and full verification details'}</a></div><div class="download-note" id="windows-preview-verification"><strong>Windows ${windowsPreview.version} · Self-Signed Preview</strong><br>${zh?'仅用于自有或 IT 批准的测试环境。安装器不会自动导入证书。安装包 SHA-256：':'For owned or IT-approved test environments only. The installer never imports certificates automatically. Installer SHA-256: '}<code>${windowsPreview.installerSha256}</code><br><a href="${windowsPreview.page}" ${analyticsAttributes('view_release_details',{ ctaId: 'download_windows_preview_details', ctaLocation: 'release_note', pageType: 'download', locale, destination: 'github_release', platform: 'windows', releaseVersion: windowsPreview.version })}>${zh?'查看便携版、公开证书、风险说明与全部校验信息':'View the portable build, public certificate, warnings, and all verification details'}</a></div><div class="download-note" id="code-signing-policy"><strong>${zh?'代码签名政策':'Code signing policy'}</strong><br>${zh?'SignPath 申请仍在等待批准；当前 Windows Preview 使用项目专用自签名证书，不是 SignPath Foundation 证书。预览版不会被描述为公众可信签名，也不替代正式发布签名门禁。':'The SignPath application is pending. The current Windows Preview uses a project-controlled self-signed certificate, not a SignPath Foundation certificate. It is not publicly trusted and does not satisfy the stable release-signing gate.'}<br><a href="${codeSigningPolicyUrl}">${zh?'阅读完整代码签名政策':'Read the complete code signing policy'}</a></div>${supporterTeaser(locale, 'download', 'download_page')}</div></section>
