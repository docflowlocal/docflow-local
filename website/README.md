...
After a visitor explicitly accepts optional analytics, GA4 records an `ai_referral_landing` event for recognized visits from ChatGPT, Perplexity, Claude, Copilot, and Gemini. Before consent—or when Do Not Track is enabled—the Google tag is not loaded and events are not sent. The footer keeps a persistent analytics-settings control. Download, beta, source, pricing, and primary navigation CTAs carry a common event context (`cta_id`, `cta_location`, `page_type`, `locale`, and destination metadata). Public-release installer downloads use `download_mac_installer` and `download_windows_installer`; they also include `file_name`, `file_extension`, `link_url`, and `link_domain`. The macOS event remains active for reporting continuity. The Windows public-release event is enabled only after a publicly trusted signed installer URL is attached to a public release.

The separately labelled self-signed Windows Preview uses `download_windows_preview`, with `cta_id=download_windows_self_signed_preview`, `platform=windows`, `asset_type=exe`, `destination=github_release_asset`, and its preview `release_version` (initially `0.6.1-preview.2`). It shares the four file-download fields above and emits the common `cta_click` with `source_event=download_windows_preview`; it never emits `download_windows_installer`. Keep Preview downloads separate from public-release installer conversions in GA4 reports. Preview tracking uses the same explicit-consent and Do Not Track rules, and stops after analytics consent is withdrawn. Run `npm run check:analytics` to verify both language contexts, event separation, file metadata, and the consent/DNT cases.
...
No DocFlow-hosted website form collects personal data in the initial release; optional payment details are entered on Stripe's hosted checkout under the disclosure in the repository-level `PRIVACY.md`. Standard Community download CTAs lead to publicly trusted signed releases. A self-signed Windows Preview, when available, is offered separately and is not a publicly trusted release. Until a publicly trusted signed Windows installer is public, the Windows card continues to emit `beta_request` for its project-email link; that email link is never counted as either an installer or Preview download.
